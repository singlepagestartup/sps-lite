...
import { Component as AdminFormInputs } from "@sps/sps-rbac-models-subject-frontend-component-variants-sps-lite-admin-form-inputs";
import { variants } from "@sps/sps-rbac-models-subject-contracts";
...
export function Component(props: IComponentPropsExtended) {
  ...
  useEffect(() => {
    if (updateEntityResult.data || createEntityResult.data) {
      dispatch(api.rtk.util.invalidateTags(["subject"]));
      invalidateServerTag({ tag: "subject" });
      ...
      <Form {...form}>
        <Card className="admin-edit-card">
          <h1 className="admin-edit-card-heading">
            {props.data?.id ? "Edit" : "Create"} subject
          </h1>
